...
public:
    void setName(const char* name) {
        if (!name or strlen(name) < 2 or strlen(name) >= SIZE) {
            throw invalid_argument("Error! Некоректне ім'я");
        }
    ...
    void setCategory(const char* category) {
        if (!category or strlen(category) < 2 or strlen(category) >= SIZE) {
            throw invalid_argument("Error! Некоректна категорія");
        }
    ...
    void setPrice(int price) {
        if (price <= 0) {
            throw invalid_argument("Error! Ціна повинна бути додатною");
        }
        this->price = price;
    }
    ...
    void setQuantity(int quantity) {
        if (quantity < 0) {
            throw invalid_argument("Error! Кількість не може бути від'ємною");
        }
    ...
    virtual void print() const {
        cout << "\t\t Інформація про товар" << endl;
        cout << "Назва             :: " << this->name << endl;
        cout << "Категорія         :: " << this->category << endl;
        cout << "Ціна              :: " << this->price << " грн" << endl;
        cout << "Кількість         :: " << this->quantity << endl;
    }
    ...
    virtual void deserialize(ifstream& file) {
        ...
        if (price <= 0 or quantity < 0) {
            throw runtime_error("Некоректні дані в файлі");
        }
    }
...
public:
    ...
    void deserialize(ifstream& file) override {
        ...
        if (!data.isValidDate()) {
            throw runtime_error("Некоректна дата в файлі");
        }
    }
};
...
public:
    ...
    void delete_(const char* name) {
        ...
        if (it != supermarketInventory.end()) {
            ...
            cout << "Товар з назвою \"" << name << "\" не знайдено." << endl;
        }
    }
    ...
    void selling(const char* name, int amount) {
        ...
        if (it != supermarketInventory.end()) {
            ...
            if (goods->getQuantity() >= amount) {
                ...
                cout << "Продаж товару \"" << name << "\" успішний!" << endl;
            }
            else {
                cout << "Недостатньо товару \"" << name << "\" на складі." << endl;
            }
        }
        else {
    ...
    void download(ifstream& file) {
        ...
        if (file.tellg() == 0) { 
            cout << "Файл порожній. Завантаження не виконано." << endl;
            return;
        }
        ...
        if (!file) {
            throw runtime_error("Некоректний формат файлу або файл пошкоджено");
        }
        ...
        cout << "Завантаження завершено успішно." << endl;
    }
